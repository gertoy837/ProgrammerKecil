...
export default function ProductListPage() {
  ...
  let filteredProducts = products.filter(product => {
    ...
    const matchesCategory = selectedCategory === 'All Product' ? true : product.category?.name === selectedCategory;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 antialiased flex flex-col justify-between font-sans">
      {/* Memanggil Google Font & Fonts Stylesheet CDN Resmi */}
        ...
        <section className="relative h-72 mb-12 rounded-3xl overflow-hidden flex flex-col items-center justify-center text-center shadow-md bg-slate-900">
          ...
          <div className="relative z-20 px-4 max-w-3xl mx-auto">
            ...
            <p className="text-xs md:text-sm text-white/80 max-w-md mx-auto mb-6 leading-relaxed">
              Curating the world's most vibrant and rare Betta varieties for connoisseurs and enthusiasts alike.
            </p>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 sticky top-24">
              <h3 className="text-sm font-extrabold text-slate-900 tracking-wide mb-4">Category</h3>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${
                        isActive ? 'bg-[#4648d4] text-white' : 'bg-slate-100 text-slate-400'
                      }`}>
              ...
              <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Sort By Price</h3>
                <select
                  value={sortBy}
            ...
            {currentItems.length === 0 ? (
              <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
                ...
                <p className="text-slate-500 font-semibold">
                  {searchQuery ? `No results found for "${searchQuery}"` : `No betta fish available.`}
                </p>
              </div>
            ) : (
              <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
                  {currentItems.map((product) => {
                    const imageUrl = product.image 
                      ...
                      : "https://via.placeholder.com/300?text=Cupang+BettaVerse";

                    return (
                      <div key={product.id} className="bg-white rounded-3xl p-4 border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 group flex flex-col justify-between">
                        <div>
                          {/* Image Wrapper */}
                          <div className="relative aspect-square rounded-2xl overflow-hidden mb-4 bg-slate-50">
                            <img 
                              ...
                              src={imageUrl}
                              onError={(e) => {
                                e.target.src = "https://via.placeholder.com/300?text=Cupang+BettaVerse";
                              }}
                            />
                            {product.stock > 0 ? (
                          ...
                          <div className="px-1">
                            ...
                            <p className="text-xs text-slate-400 mb-4 line-clamp-2 leading-relaxed">
                              {product.description || "Premium quality curated specimen."}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="px-1 mt-auto">
                          ...
                          <button className="w-full py-3 bg-[#4648d4] hover:bg-[#3b3dbb] text-white rounded-xl font-bold text-xs transition-all active:scale-95 flex items-center justify-center gap-2 shadow-sm">
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-3 mt-12">
                    ...
                    <span className="text-xs font-semibold text-slate-400">
                      Page <span className="text-slate-800 font-bold">{currentPage}</span> of {totalPages}
                    </span>
                    <button
